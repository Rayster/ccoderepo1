...
const PANELS = [
  ...
  },
  {
    ...
    color: '#f97316',
    visual: 'wave',
    caption: 'Voice, translation, speech',
  },
  {
    label: 'LEARN',
    icon: '⬡',
...
const PanelVisual = ({ type, color, frame }: { type: string; color: string; frame: number }) => {
        ...
        {Array.from({ length: 5 }, (_, i) => (
          <div
            ...
            style={{
              height: 5,
              background: color,
              opacity: 0.5 + 0.3 * sr(i * 31 + frame * 0.1),
              borderRadius: 2,
              marginBottom: 6,
              width: `${50 + sr(i * 47) * 40}%`,
            }}
        ...
        {bars.map((h, i) => (
          <div
            ...
            style={{
              ...
              height: `${h * 100}%`,
              background: color,
              opacity: 0.6 + 0.2 * Math.sin(frame * 0.12 + i),
  ...
  if (type === 'wave') {
    const pts = Array.from({ length: 40 }, (_, i) => ({
      x: (i / 39) * 160,
      y: 28 + Math.sin((i / 39) * Math.PI * 5 + frame * 0.2) * 18 * Math.sin((i / 39) * Math.PI),
    }));
  ...
  if (type === 'shapes') {
    return (
      <svg viewBox="0 0 160 56" style={{ width: '100%', height: 56 }}>
        ...
        <rect x="60" y="14" width="28" height="28" rx="4" fill={color} opacity={0.2} stroke={color} strokeWidth="1"
          transform={`rotate(${frame * 0.5}, 74, 28)`} />
      ...
      <svg viewBox="0 0 160 56" style={{ width: '100%', height: 56 }}>
        {edges.map(([a, b], i) => (
          <line
            ...
            stroke={color}
            strokeWidth="1"
            opacity={0.4 + 0.3 * Math.sin(frame * 0.1 + i)}
          />
        ))}
        {nodes.map(([x, y], i) => (
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG, overflow: 'hidden', fontFamily: 'Courier New, monospace' }}>
      ...
      >
        ...
        <div style={{ fontSize: 28, fontWeight: 900, color: '#fff', letterSpacing: '0.06em' }}>
        ...
        {PANELS.map((panel, i) => {
          ...
          return (
            <div
              key={panel.label}
              style={{
                border: `1px solid ${panel.color}${isActive ? 'aa' : '33'}`,
                borderRadius: 6,
                background: `${panel.color}${isActive ? '0d' : '05'}`,
                padding: 14,
                opacity: panelOpacity,
                boxShadow: isActive ? `0 0 20px ${panel.color}22, inset 0 0 20px ${panel.color}08` : 'none',
                display: 'flex',
